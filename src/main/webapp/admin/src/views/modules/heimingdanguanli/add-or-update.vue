<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="拉黑时间" prop="laheishijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.laheishijian" 
                type="datetime"
                placeholder="拉黑时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.laheishijian" label="拉黑时间" prop="laheishijian">
              <el-input v-model="ruleForm.laheishijian" 
                placeholder="拉黑时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.xingming = true;
          ...
          if(o=='laheishijian'){
            this.ruleForm.laheishijian = obj[o];
	    this.ro.laheishijian = true;
            continue;
          }
          if(o=='beizhu'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `heimingdanguanli/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.heimingdanguanliCrossAddOrUpdateFlag = false;
                  this.parent.search();
