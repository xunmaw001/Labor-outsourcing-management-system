<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="单位负责人" prop="danweifuzeren">
          <el-input v-model="ruleForm.danweifuzeren" 
              placeholder="单位负责人" clearable  :readonly="ro.danweifuzeren"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="单位负责人" prop="danweifuzeren">
              <el-input v-model="ruleForm.danweifuzeren" 
                placeholder="单位负责人" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.fapiaopingzheng" label="发票凭证" prop="fapiaopingzheng">
          <file-upload
          tip="点击上传发票凭证"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.fapiaopingzheng?ruleForm.fapiaopingzheng:''"
          @change="fapiaopingzhengUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.fapiaopingzheng" label="发票凭证" prop="fapiaopingzheng">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.fapiaopingzheng.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="员工名单" prop="yuangongmingdan">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.yuangongmingdan" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.yuangongmingdan" label="员工名单" prop="yuangongmingdan">
                    <span v-html="ruleForm.yuangongmingdan"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	yuangongmingdan : false,
	fapiaopingzheng : false,
      },
      ruleForm: {
        danweimingcheng: '',
        danweifuzeren: '',
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.danweimingcheng = true;
          ...
          if(o=='danweifuzeren'){
            this.ruleForm.danweifuzeren = obj[o];
	    this.ro.danweifuzeren = true;
            continue;
          }
          if(o=='yuangongrenshu'){
	    ...
	    this.ro.yuangongrenshu = true;
          ...
          if(o=='yuangongmingdan'){
            this.ruleForm.yuangongmingdan = obj[o];
	    this.ro.yuangongmingdan = true;
            continue;
          }
          if(o=='fapiaopingzheng'){
            this.ruleForm.fapiaopingzheng = obj[o];
	    this.ro.fapiaopingzheng = true;
            continue;
          }
        }
      }
	...
	this.ruleForm.yuangongmingdan = this.ruleForm.yuangongmingdan.replace(reg,'../../../ssmvg6r7/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yonggongdanwei/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yonggongdanweiCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    fapiaopingzhengUploadChange(fileUrls) {
	this.ruleForm.fapiaopingzheng = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
