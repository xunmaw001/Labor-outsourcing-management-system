<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="客户编号" prop="kehubianhao">
          <el-input v-model="ruleForm.kehubianhao" 
              placeholder="客户编号" clearable  :readonly="ro.kehubianhao"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="客户编号" prop="kehubianhao">
              <el-input v-model="ruleForm.kehubianhao" 
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="开发度" prop="kaifadu">
          <el-select v-model="ruleForm.kaifadu" placeholder="请选择开发度">
            <el-option
                v-for="(item,index) in kaifaduOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="开发度" prop="kaifadu">
	      <el-input v-model="ruleForm.kaifadu"
                placeholder="开发度" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.fuzeren = true;
          ...
          if(o=='kaifadu'){
            this.ruleForm.kaifadu = obj[o];
	    this.ro.kaifadu = true;
            continue;
          }
        }
      }
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.kaifaduOptions = "完成,未完成".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `kehukaifatongji/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `kehukaifatongji/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.kehukaifatongjiCrossAddOrUpdateFlag = false;
                  this.parent.search();
